...
$info = $request->GetDataUnsecure('info'); //is content for bag-info.txt

$bagman = new BagitManager($path,$config);

//Create BagIt

$error = $bagman->CreateBagit($bagit,$info);

if($error != null)
  $request->error('HTTP/1.0 400 Bad Request',$error);

$request->ok('HTTP/1.0 201 Created','bagit successfuly created', array("bagit" => $bagit));
