...
$mode = $request->GetData('mode',REGEX_MODE);
$name = $request->GetData('name',REGEX_FILENAME);
$tempname = $request->GetData('tempname',REGEX_FILENAME);
$md5 = $request->GetData('md5',REGEX_MD5);

if($mode != 'complete')
{
	 $request->error('HTTP/1.0 400 Bad Request','file append mode not supported: '.$mode);
}
//Append File

$bagman = new BagitManager($path,$config);

$error = $bagman->AppendFile($bagit,$name,$md5,$tempname);

if($error != null)
	$request->error('HTTP/1.0 400 Bad Request',$error);

$request->ok('HTTP/1.0 201 Created','file successfuly appended', array("bagit" => $bagit , "md5" => $md5));
